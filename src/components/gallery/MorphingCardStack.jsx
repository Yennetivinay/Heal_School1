...
export function MorphingCardStack({
  ...
  const [layout, setLayout] = useState(defaultLayout || "stack")
  ...
  const containerStyles = {
    stack: "relative h-64 w-64 md:h-80 md:w-80",
  ...
  return (
    <div className={cn("space-y-4", className)}>
      {/* Layout Toggle */}
      <div className="flex items-center justify-center gap-1 rounded-lg bg-slate-800/50 backdrop-blur-sm p-1 w-fit mx-auto border border-slate-700/50">
      ...
      {/* Cards Container */}
      <LayoutGroup>
        <motion.div layout className={cn(containerStyles[layout] || containerStyles.stack, "mx-auto")}>
          <AnimatePresence mode="popLayout">
            {displayCards.map((card) => {
              if (!card || !card.id) return null
              const styles = getLayoutStyles(card.stackPosition || 0)
              const isExpanded = expandedCard === card.id
              const isTopCard = layout === "stack" && (card.stackPosition || 0) === 0
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    ...
                    layout === "stack" && "absolute w-56 h-48 md:w-64 md:h-56",
